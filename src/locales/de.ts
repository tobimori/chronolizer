...
const parseRollingPeriod = (input: string) => {
  ...
  const pastDative = EffectString.match(
    /^(?:in den letzten|in den vergangenen|in den vorherigen) ([1-9]\d*) (tag|tagen|woche|wochen|monat|monaten|quartal|quartalen|jahr|jahren)$/u,
  )(input);
...
const renderGerman = (range: DateRangeExpr) => {
  ...
  if (Option.isSome(future)) {
    ...
    if (entry !== undefined) {
      return Option.some(
        `nächste ${future.value.amount} ${title(
          future.value.amount === 1 ? entry.noun : entry.plural,
        )}`,
      );
    }
  }
